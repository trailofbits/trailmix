//! Brainpool P256r1 (short-Weierstrass over the random RFC 5639 prime)
//! end-to-end EC-add test through the curve-generic driver. Uses the
//! `GenericPrime` (compare-and-subtract-q) reduction strategy.
...
mod tests {
    ...
    use crate::ec::curves::mod_arith::GenericPrime;
    use crate::ec::curves::params;
    use num_bigint::BigUint;
    use num_traits::Zero;

    /// End-to-end Brainpool P256r1 EC point addition on 64 RANDOM on-curve
    /// pairs (P, Q) with distinct x: quantum P = (x2, y2), classical
    /// Q = (ox, oy) input registers (one value per shot). Verifies
    /// (x2, y2) -> P + Q (mod q) on every shot.
    #[test]
    fn ec_add_brainpool_random_pairs() {
        let n = 256usize;
        let total = n + u_padding(n);
        let cp = params::brainpoolp256r1();
        let q = cp.p.clone();
        let mr = GenericPrime::new(256, cp.p.clone());

        let mut rng = rand::thread_rng();

        let mut circ = crate::circuit::Circuit::new();
        // measured peak 1177 (generic-prime exact reduce); +8 margin.
        ...
        eprintln!("  cost(ec_add brainpool, random pairs): ops={ops} tof={tof} peak+={peak}");
